  ...
  app.get('/purge', async (req, res) => {
    ...
    try {
      const cacheaccount = await fetch(
        `https://vps.trisout.tech/api/application/users/${req.session.pterodactyl.id}?include=servers`,
        {
          method: "get",
          headers: {
      ...
      const response = await fetch(`https://vps.trisout.tech/api/application/servers`, {
        headers: {
          Authorization: `Bearer key`,
          "Content-Type": "application/json",
      ...
      const inactiveServers = servers.filter(
        (server) => !server.attributes.name.includes("[Active]")
      );
      ...
      for (const server of inactiveServers) {
        try {
          ...
          const deleteResponse = await fetch(`https://vps.trisout.tech/api/application/servers/${serverId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer key`,
              "Content-Type": "application/json",

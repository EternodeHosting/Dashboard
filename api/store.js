...
module.exports.load = async function(app, db) {
  ...
  app.get("/buyram", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newram > 320000000) return res.send("You reached max ram limit!");
      if (newusercoins == 0) {
  ...
  app.get("/buydisk", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newdisk > 320000000) return res.send("You reached max disk limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
  ...
  app.get("/buycpu", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newcpu > 3200000000) return res.send("Reached max CPU limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
  ...
  app.get("/buyservers", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newservers > 32000000000) return res.send("Reached max server limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
